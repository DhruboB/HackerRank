package algorithm;

import java.util.Arrays;
import java.util.Comparator;
import java.util.HashMap;
import java.util.Map;

public class LargestPermutation {

    public static void main(String[] args) {
//        int[] arr = {4,2,3,5,1,6};
//        int k = 2;// 5, 2, 3, 4, 1

        int[] arr ={145,161,11,116,64,85,158,167,125,14,104,34,15,119,51,118,63,188,166,113,114,194,126,53,133,109,48,40,32,52,108,62,153,105,44,129,89,186,45,73,115,66,130,56,141,197,36,192,107,24,22,152,81,137,111,101,100,144,9,16,31,154,198,159,70,179,50,172,39,155,79,37,87,69,121,134,93,7,5,17,110,122,96,103,177,193,95,33,164,71,143,82,77,75,162,191,102,19,91,94,157,184,35,23,99,180,182,6,59,176,146,13,165,135,3,4,195,112,189,30,27,168,123,41,57,136,190,29,132,76,38,1,10,83,124,163,20,117,178,2,42,84,12,171,67,43,58,183,8,138,68,149,131,47,74,60,80,90,78,160,169,21,61,187,92,25,181,147,88,106,55,97,150,120,128,139,140,175,26,127,173,185,65,196,86,28,98,54,142,18,151,46,49,170,174,156,148,72};
        int k = 162 ;// ?
        System.out.println(Arrays.toString(largestPermutation(k,arr)));
    }

    // Complete the largestPermutation function below.
    static int[] largestPermutation(int k, int[] arr) {
        // create a duplicate
        // sort the duplicate
        // compare and replace in duplicate by mainting original positions
        int len = arr.length;
        int[][] sorted = new int[len][2];
        for(int i=0 ; i < len ; i++){
            sorted[i][0] = arr[i];
            sorted[i][1] = i;
        }
        Arrays.sort(sorted, Comparator.comparing(o -> o[0]));
        int temp = 0;
        // need O(1) look up for original positions
        Map<Integer, Integer> map = new HashMap<>();
        for(int i=0; i < len ; i++){
            map.put(sorted[i][0],sorted[i][1]);
        }
        // k could be more than len
        int maxSwap = k > len ? len : k;
        for(int i=0 ; i < len && maxSwap > 0; i++){
            if( arr[i] != sorted[len - 1 - i][0]){
                temp = arr[i]; // swap max with arr[i]
                arr[i] = sorted[len - 1 - i][0];
                arr[map.get(sorted[len - 1 - i][0])] = temp;
                map.put(temp,map.get(arr[i])); // modify original positions for old value
                map.put(arr[i],i); // modify original positions for Max value
                maxSwap--; // reducing remaining swap count
            }
        }
        return arr;
    }
}
